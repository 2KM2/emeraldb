...
int pmdOptions::init ( int argc, char **argv )
{
   ...
   if ( rc )
   {
      OSS_LOG ( LOG_ERROR, "Failed to read cmd, rc = %d", rc ) ;
      goto error ;
   }
   ...
   if ( rc )
   {
      OSS_LOG ( LOG_ERROR, "Unexpected error when reading conf file, rc = %d",
               rc ) ;
      goto error ;
   }
   // load vm from file
   ...
   if ( rc )
   {
      OSS_LOG ( LOG_ERROR, "Failed to import from vm2, rc = %d", rc ) ;
      goto error ;
   }
   ...
   if ( rc )
   {
      OSS_LOG ( LOG_ERROR, "Failed to import from vm, rc = %d", rc ) ;
      goto error ;
   }
done :
   return rc ;
